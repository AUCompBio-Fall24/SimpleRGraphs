...
This tutorial was adapted from [Frank McCown's](https://sites.harding.edu/fmccown/r/) site on simple graphs in R. Once the tutorial and lab assignment are complete, students should be familiar with the **following learning objectives**:
...
Within tidyverse are a multitude of useful packages! 
Below we are piping `%>%` our dataset into dplyr's `filter` function. 
Don't worry if you don't understand this syntax, you haven't learned about `dplyr` in this course!
...
The code below accomplishes the same goal. 
Remove any unused objects, and use line 10 to ensure that data from the third tree in our subset,`Tree13`,  is included in the graph:
...
box()
lines(santrees$Tree5, type = "l", col = plotcols[2],lty=2, lwd = 3)
lines(santrees$Tree13, type = "l", col = plotcols[3],lty=3, lwd = 3)
title(main = "Tree Heights Across 11 Years ", col.main = "navy", font.main = 4)
legend("bottomright", names(santrees), cex=0.8, col=plotcols, 
   lty=1:3, lwd=3, bty="n")
```
...
Let's look at multiple (3) trees with `santrees` we made in the previous section:
...
Later in the tutorial, we will explore how to add the legend adjacent to the graph, so that we don't 
worry about our legend covering features of our graph. You can also change the margins of the plot using the `par` function
...
I'd like to add the mean value for the data to the histogram. We can accomplish this with the `abline` function:
```{r abl-his, exercise=TRUE, exercise.eval=FALSE, exercise.setup =  "fin-bar-subset"}
allyr <- c(tresages$Yr19, tresages$Yr15, tresages$Yr12)

# Get mean of allyr into an object
mntree<- mean(allyr)

hist(allyr, col=colpkr[48], ylim=c(0,20), main = "Histogram of Heights for Ages 12,15,& 17",
     xlab = "Height (Meters)")
# Use abline to add the mean to the histogram; using "v =" because we want a vertical line
abline(v = mntree, col = colpkr[36], lwd = 2)
```

I am not fond of the spacing at my axes, so let's turn off the `axes =` argument within `hist()` and used the `axis` function so that we can manipulation positioning with the `pos =` argument:

```{r abl-ax-his, exercise=TRUE, exercise.eval=FALSE, exercise.setup =  "fin-bar-subset"}
allyr <- c(tresages$Yr19, tresages$Yr15, tresages$Yr12)
