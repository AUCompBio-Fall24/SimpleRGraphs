...
### Choosing Bins
Build on the previous graph, this time forcing each height into its own bin by setting 
`breaks` to the max height in the data:
...
# Single bin histogram with horizontal y-axis labels
  ...
  # right=F means that the cells are right open intervals,
  # i.e. from [1 to 2], included values from 1 up until and including (1..1.98,1.99... & 2)
hist(allyr, col=heat.colors(mxy), breaks=mxy, ylim=c(0,5), xlab = "Height (Meters)",
   xlim=c(0,mxy), right=F, main="Histogram of Heights for Ages 12,15,& 17", las=1)
...
Plotting the log(normal distribution) for 1000 random numbers
...
plot(h$counts, log="xy", pch=23, col=colpkr[459],
